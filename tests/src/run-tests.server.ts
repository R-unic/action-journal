...
testRunner.run({ colors: true })
  .then(() => {
    ...
    let [reportJSON] = unmappedJSON.gsub('ReplicatedStorage/Library/([^"]+)', 'out/%1.lua');
    [reportJSON] = reportJSON.gsub('ReplicatedStorage/Library', 'out/init.lua');
    [reportJSON] = reportJSON.gsub(`ReplicatedStorage/rbxts_include/node_modules/@rbxts/${libName}/([^"]+)`, 'out/%1.lua');
    [reportJSON] = reportJSON.gsub(`ReplicatedStorage/rbxts_include/node_modules/@rbxts/${libName}`, 'out/init.lua');
